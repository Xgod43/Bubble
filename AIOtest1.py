...
def parse_selection(raw):
	...
	result = []
	for item in items:
		if item in TESTS and item not in result:
			result.append(item)
	return result
...
def main():
	print("All-in-One Raspberry Pi Hardware Tester")
	print("Choose tests:")
	print("  1 = Stepper")
	print("  2 = Limit switches")
	print("  3 = Pressure sensor")
	print("  4 = Pi camera")
	print("  5 = Load cell")
	print("  all = run all tests in order")
	print("  custom example: 2,4")
	...
	print("Running tests:", ", ".join(f"{tid}:{TESTS[tid]}" for tid in selected))
	for tid in selected:
		try:
			run_test_by_id(tid)
		except Exception as exc:
			print(f"Test {tid} failed: {exc}")
