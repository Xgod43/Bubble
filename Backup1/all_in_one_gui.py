				...
				if view_type == "binary":
					...
					if flow_prev_gray is not None and flow_prev_pts is not None:
						...
						if next_pts is not None and status is not None:
							...
							for old_pt, new_pt in zip(good_old, good_new):
								...
								if view_type == "optical_flow_2d":
									cv2.arrowedLine(display_bgr, (int(ox), int(oy)), (int(nx), int(ny)), (0, 255, 255), 1, tipLength=0.25)
							...
							if flow_mags:
								...
								distance_text = f"Distance mean: {flow_distance_mean:.3f} {unit} | max: {flow_distance_max:.3f} {unit}"
								if view_type == "optical_flow_2d":
									cv2.putText(display_bgr, distance_text, (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 245, 80), 2)
							else:
						...
						if view_type == "optical_flow_3d" and flow_prev_gray is not None:
							dense_flow = cv2.calcOpticalFlowFarneback(flow_prev_gray, flow_gray, None, 0.5, 3, 21, 3, 5, 1.2, cv2.OPTFLOW_FARNEBACK_GAUSSIAN)
							...
							distance_text = f"Distance mean: {flow_distance_mean:.3f} {unit} | max: {flow_distance_max:.3f} {unit}"
							cv2.putText(display_bgr, distance_text, (12, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.60, (250, 245, 85), 2)

						flow_prev_gray = flow_gray
					elif view_type == "mosaic":
						display_bgr = pipe.build_mosaic(pre, binary, vector_vis, heatmap_vis, pointcloud_vis=pointcloud_vis if params["use_pointcloud"] else None, scale=params["mosaic_scale"])
